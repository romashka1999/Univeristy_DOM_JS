...
export const drawStudents = (uni, students) => {
    ...
    for(let student of students) {
        ...
        button.addEventListener('click', function () {
            const res =  window.confirm('are you sure ? do you want  ?', )
            if(res) {
        ...
        textNode = document.createTextNode('cancel registration');
        button.appendChild(textNode);
        tr.appendChild(button);
        arr.push(tr);
    }
    studentTableSearch.innerHTML = '';
    for(let tr of arr) {
        studentTableSearch.appendChild(tr);
    }
}
